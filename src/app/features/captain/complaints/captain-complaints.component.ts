              ...
              <ng-container matColumnDef="action">
                ...
                <td mat-cell *matCellDef="let complaint" class="py-4 whitespace-nowrap text-right border-b border-gray-200">
                  <div class="flex items-center justify-end gap-2">
                    ...
                    <ng-container *ngIf="complaint.status !== 'resolved' && complaint.status !== 'rejected' && !complaint.is_escalated">
                    ...
                    <ng-container *ngIf="complaint.status === 'resolved' || complaint.status === 'rejected' || complaint.is_escalated">
                      <div class="w-36 px-3 py-1.5 border-2 border-gray-900 rounded-sm text-xs font-black uppercase tracking-wider text-center flex items-center justify-center gap-1 shadow-[1px_1px_0px_0px_rgba(17,24,39,1)]"
                           [ngClass]="complaint.is_escalated ? 'bg-red-100 text-red-900' : (complaint.status === 'rejected' ? 'bg-red-50 text-red-900' : 'bg-green-100 text-green-900')">
                        <mat-icon class="scale-75 -ml-1">
                          {{ complaint.is_escalated ? 'whatshot' : (complaint.status === 'rejected' ? 'lock' : 'check_circle') }}
                        </mat-icon>
