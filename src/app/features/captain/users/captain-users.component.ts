...
import { AdminUserDetailDialogComponent } from '../../admin/users/admin-user-detail-dialog.component';
  ...
  template: `
    <div class="p-6 max-w-7xl mx-auto pb-20">
      <div class="flex justify-between items-center mb-6">
        <div>
          <h1 class="text-3xl font-black text-gray-900 uppercase tracking-tight" style="font-family: 'Arial Black', Impact, sans-serif;">Barangay Citizens</h1>
          <p class="text-sm font-bold text-gray-600 uppercase tracking-wider">View citizens registered in your barangay</p>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 mb-8 bg-gray-50 border-2 border-gray-900 p-4 rounded-sm shadow-[4px_4px_0px_0px_rgba(17,24,39,1)]">
        <mat-form-field appearance="outline" class="w-full bg-white filter-field">
...
export class CaptainUsersComponent implements OnInit {
  ...
  async loadUsers() {
    ...
    // First get the captain's own barangay
    ...
    // Then fetch citizens strictly in that barangay
    ...
    if (!error && data) {
      ...
      const pathsToSign = this.users
        ...
        .map(u => {
          ...
          // The database currently stores paths like `userId/timestamp.jpeg`.
          // If the path does not include 'citizen_ids/', we assume it's already just the folder/file path.
          return path;
        });
          ...
          this.users.forEach(u => {
            if (u.proof_of_residency_url) {
              ...
              if (match?.signedUrl) {
                ...
                console.warn('Could not generate signed URL for:', path);
                // We should NOT leave the raw path in proof_of_residency_url, because the HTML <img> tag
                // will try to load it as a relative URL (e.g. localhost:4200/userId/image.jpg), causing a 404.
                u.proof_of_residency_url = null; 
              }
            }
          });
        }
  ...
  applyFilters() {
    this.filteredUsers = this.users.filter(user => {
      return this.statusFilter === 'all' || user.verification_status === this.statusFilter;
    });
